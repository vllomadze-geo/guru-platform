...
function ensureProductSegmentGlobals(workspace = state) {
  ...
  workspace.project = workspace.project || {};
  workspace.sharedEvidence = workspace.sharedEvidence || {};
  const card = (workspace.gates || []).flatMap(g => g.cards || []).find(isProductSegmentClientCard);
  if (card) {
    card.instruction = 'Инструменты: бриф, интервью, CRM, сайт, отдел продаж.\n\nБлок отвечает за понимание: что продаётся, кому продаётся и какую задачу клиента решает.\n\nНа выходе должно быть понятно: что продаём, кому и ради какого результата.';
    card.evidenceFields = PRODUCT_SEGMENT_GLOBAL_FIELDS.map(field => ({ key: field.evidenceKey, label: field.label }));
  }
...
function productSegmentStatus(workspace = state) {
  const values = PRODUCT_SEGMENT_GLOBAL_FIELDS.map(field => String(productSegmentValue(field, workspace) || '').trim());
  const filled = values.filter(Boolean).length;
  if (!filled) return 'not_started';
...
function updateProductSegmentField(e) {
  const field = PRODUCT_SEGMENT_GLOBAL_FIELDS.find(item => item.key === e.target.dataset.productSegmentField);
...
isSystemSpecialCard = function(card) {
  return isProductSegmentClientCard(card) || __guruPrevIsSystemSpecialCardV11(card);
};
...
textValuesForStatus = function(card, workspace = state) {
  if (isProductSegmentClientCard(card)) return PRODUCT_SEGMENT_GLOBAL_FIELDS.map(field => productSegmentValue(field, workspace));
  return __guruPrevTextValuesForStatusV11(card, workspace);
};

const __guruPrevFormatStructuredEvidencePlainV11 = formatStructuredEvidencePlain;
...
cardUserFieldsHtml = function(c) {
  if (isProductSegmentClientCard(c)) return `<div class="field-row product-segment-row"><span>Глобальные данные проекта</span>${productSegmentFieldsHtml(c)}</div>`;
