...
const FIELD_STANDARDS = {
  ...
  продукт_одно_предложение: {
    label: 'Продукт',
    formula: 'Одно предложение: что вы продаёте и почему это существует. Не список характеристик',
    example: 'Химчистка мягкой мебели на дому — возвращаем мебели вид нового за один визит',
    reference: 'Джобс: «Простота — это высшая форма изысканности». Если нужен абзац — вы не закончили думать',
    min: 20, max: 200
  },
  сегмент_кто_и_в_какой_ситуации: {
    label: 'Сегмент',
    formula: 'Конкретный человек в конкретной ситуации — не «женщины 25–45», а момент потребности',
    example: 'Владелец квартиры, у которого гости через 3 дня и пятно на диване',
    reference: 'Мюллер-Брокман: форма следует за содержанием. Сегмент = ситуация, не демография',
    min: 15, max: 200
  },
  задача_клиента_что_хочет_изменить: {
    label: 'Задача клиента',
    formula: 'Его собственные слова — что он хочет изменить. Не ваша интерпретация, а его реальность',
    example: 'Хочу, чтобы диван выглядел чистым к приходу гостей, и не думать об этом',
    reference: 'Джобс: «Начните с клиентского опыта и двигайтесь назад к технологии»',
    min: 15, max: 200
  },
  результат_состояние_после: {
    label: 'Результат',
    formula: 'Что изменится, когда работа выполнена. Не процесс — состояние после',
    example: 'Диван как новый, запаха нет, можно принимать гостей',
    reference: 'JTBD: клиент покупает не дрель, а дырку в стене. Не чистку — а чистый диван',
    min: 10, max: 200
  },
  queries: {
    label: 'Основные запросы',
    formula: 'Собрать первичное ядро из Wordstat, Search Console и отчётов по поисковым запросам',
    example: 'химчистка дивана на дому — 3 200/мес — коммерческий',
    reference: 'Фиксирует существующий спрос в формулировках самих пользователей',
    min: 20, max: 500
  },
  frequency: {
    label: 'Частотность / динамика',
    formula: 'Проверить динамику, региональность и связанные формулировки по каждому кластеру',
    example: '3 200/мес, стабильный, пик — сентябрь–ноябрь',
    reference: 'Не смотреть запрос изолированно — сравнивать кластеры между собой',
    min: 10, max: 300
  },
  intent: {
    label: 'Интент',
    formula: 'Разложить запросы по типам намерений: купить / выбрать / сравнить / понять / записаться',
    example: 'Коммерческий: ищет исполнителя, готов заказать, сравнивает цены',
    reference: 'Один кластер = один доминирующий интент. Убрать смешение',
    min: 10, max: 300
  },
  conclusion: {
    label: 'Вывод',
    formula: '[Главный факт из данных] + [что это значит для проекта] + [рекомендация]',
    example: 'Спрос стабильный, 80% — коммерческий. Можно запускать поисковую рекламу.',
    reference: 'Вывод = мост между данными и решением',
    min: 20, max: 400
  },
  competitors: {
    label: 'Основные конкуренты',
    formula: 'Таблица: конкурент + заголовок + УТП + цена + сроки + доказательства + CTA',
    example: 'CleanHome — «Чистка за 2 часа» — 4500₽ — гарантия — «Заказать»',
    reference: 'Сравнивать не бренды, а предложения внутри конкретного кластера спроса',
    min: 20, max: 500
  },
  repeatedPromises: {
    label: 'Повторяющиеся обещания',
    formula: 'Выделить формулы, которые повторяют все: «быстро», «качественно», «под ключ», «с гарантией»',
    example: 'Все обещают «качественную химчистку», «опытных мастеров», «современное оборудование»',
    reference: 'Всё, что повторяют все — редко даёт сильную дифференциацию',
    min: 20, max: 400
  },
  weakSpots: {
    label: 'Слабые места конкурентов',
    formula: 'Какие боли описаны слабо, какие доказательства отсутствуют, где нет ясного сценария действия',
    example: 'Нет гарантии результата, долгое ожидание (2–3 дня), непрозрачные цены',
    reference: 'Искать не слабый дизайн, а слабую аргументацию и недоработанные сценарии выбора',
    min: 15, max: 400
  },
  differentiation: {
    label: 'Точка отличия',
    formula: 'Вывести отличие из карты конкурентов и пустот в категории — значимое для выбора',
    example: 'Выезд за 2 часа, а не за 2 дня — важно для тех, кто ждёт гостей',
    reference: 'Отличие должно быть значимо для выбора, не просто «индивидуальный подход»',
    min: 20, max: 300
  },
  explicitPains: {
    label: 'Явные боли',
    formula: 'Выписать боли, которые прямо читаются из запросов: «не работает», «дорого», «срочно», «как выбрать»',
    example: 'Пятно на диване не выводится, запах не уходит, стыдно перед гостями',
    reference: 'Сначала фиксировать буквальные боли рынка, а не креативные интерпретации команды',
    min: 15, max: 400
  },
  hiddenPains: {
    label: 'Скрытые боли',
    formula: 'Для каждого кластера: «что случилось до этого запроса» и «что человек боится получить после выбора»',
    example: 'Боится испортить мебель ещё больше, не знает кому доверять, откладывает',
    reference: 'Скрытую боль выводить только из контекста спроса и конкурентного обещания',
    min: 15, max: 400
  },
  reason: {
    label: 'Причина спроса',
    formula: 'Для каждого кластера указать событие-триггер: поломка, срочность, сравнение подрядчиков, сложный выбор',
    example: 'Ребёнок пролил сок, переезд, подготовка к продаже квартиры',
    reference: 'Причина спроса важнее абстрактного портрета клиента',
    min: 10, max: 300
  },
  mainPain: {
  ...
  functionalJob: {
    ...
    formula: 'Закончить: «Когда возникает [ситуация], мне нужно [прогресс], чтобы [результат]»',
    example: 'Когда на диване пятно — мне нужно убрать его, чтобы мебель выглядела чистой',
    reference: 'Формулировать как прогресс в ситуации, а не как функцию продукта',
    min: 15, max: 300
  },
  emotionalJob: {
    label: 'Эмоциональная работа',
    formula: 'Что клиент хочет убрать: тревогу, стыд, неопределённость, усталость, страх ошибки',
    example: 'Спокойствие, что дом чистый; уверенность перед приходом гостей',
    reference: 'Не подменять эмоцию банальным «хочу удобно»',
    min: 10, max: 300
  },
  socialJob: {
    label: 'Социальная работа',
    formula: 'Что человек хочет показать или избежать перед семьёй, коллегами, клиентами, руководством',
    example: 'Хозяйка, которая следит за домом; ответственный арендатор',
    reference: 'Особенно важно в премиуме, B2B и чувствительных категориях',
    min: 10, max: 300
  },
  mainJtbd: {
    label: 'Главный JTBD',
  ...
  segments: {
    ...
    formula: 'Разбить рынок по интенту, срочности, риску, чеку, уровню доверия, типу результата',
    example: 'Мамы с детьми — срочно, цена вторична. Арендаторы — перед сдачей, ищут дешевле.',
    reference: 'Строить ЦА не по полу и возрасту, а по ситуации спроса',
    min: 20, max: 500
  },
  personas: {
    label: 'Персоны',
    formula: 'На каждый сегмент: 1–2 профиля с задачей, триггером, страхом и критерием выбора',
    example: 'Марина, 35 — мама, пятно от сока, нужен выезд сегодня, цена вторична',
    reference: 'Персоны — рабочий инструмент, а не декоративный маркетинг',
    min: 20, max: 500
  },
  triggers: {
  ...
  partners: {
    ...
    formula: 'Тип партнёра + аудитория + точка пересечения с нашей ЦА',
    example: 'Мебельные магазины — покупатели нуждаются в уходе за новой мебелью',
    reference: 'Рассматривать только тех, чья аудитория реально пересекается',
    min: 15, max: 400
  },
  partnerBenefit: {
  ...
  format: {
    ...
    formula: 'Что делает компания + что делает партнёр + какой путь проходит клиент',
    example: 'Партнёр даёт флаер → мы даём скидку 10% → партнёр получает % от заказа',
    reference: 'Сразу выбирать механику: акция, промокод, QR, подарок, пакет',
    min: 10, max: 300
  },
  priority: {
    label: 'Приоритет',
  ...
  value_proposition: {
    ...
    formula: 'Что продвигаем (услуга / симптом / результат) — чётко зафиксировать до сбора семантики',
    example: 'Химчистка мягкой мебели на дому для владельцев квартир — результат за 2 часа',
    reference: 'Список минус-зон обязателен до сбора семантики',
    min: 20, max: 300
  },
  limitations: {
    label: 'Ограничения и исключения',
    formula: 'Что НЕ продвигаем: бесплатно, онлайн, вакансии, обучение, DIY — список до запуска',
  ...
  phrases: {
    ...
    formula: 'Wordstat + подсказки + «Похожие запросы»: разговорные формулировки, жаргон, ошибки — без фильтрации',
    example: '«почистить диван», «вывести пятно с дивана», «химчистка на дом»',
    reference: 'Перевести продукт на язык клиента: как он сам формулирует проблему',
    min: 15, max: 400
  },
  pain_words: {
    label: 'Слова боли',
    formula: 'Слова, которые клиент использует при описании проблемы — из запросов и отзывов',
    example: '«воняет», «не отстирывается», «испортили», «страшно трогать»',
    min: 10, max: 300
  },
  commercial_words: {
    label: 'Коммерческие уточнения',
    formula: 'Слова готовности к покупке: «заказать», «цена», «стоимость», «на дому», «с выездом»',
    example: '«заказать», «цена», «стоимость», «на дому», «с выездом», «недорого»',
    min: 10, max: 300
  },
  hot_intents: {
    label: 'Горячие интенты',
    formula: 'Запросы с максимальной готовностью к действию — приоритет при ограниченном бюджете',
    example: '«химчистка дивана заказать», «вызвать химчистку на дом цена»',
    min: 15, max: 400
  },
  compare_intents: {
    label: 'Интенты сравнения',
    formula: 'Запросы, где клиент сравнивает / выбирает — требуют другой посадочной с доказательствами',
    example: '«химчистка мебели отзывы», «лучшая химчистка Москва»',
    min: 10, max: 400
  },
  trash_intents: {
    label: 'Мусорные интенты',
    formula: 'Запросы без коммерческого интента → минус-слова. Без них бюджет сгорает',
    example: '«химчистка дивана своими руками», «средство для чистки», «вакансии»',
    min: 10, max: 400
  },
  cluster_logic: {
    label: 'Правило группировки',
    formula: '1 кластер = 1 тема = 1 интент. Не дробить без необходимости',
    example: 'По типу мебели: диваны, ковры, матрасы — разные услуги, разные посадочные',
    reference: 'Узкая ниша: старт с 1 группы. Широкий спрос: 3–8 групп по подтемам',
    min: 15, max: 400
  },
  landing_match: {
    label: 'Соответствие посадочным',
    formula: '[Группа запросов → какая страница] + [есть ли она]',
    example: '«химчистка дивана» → /uslugi/divany, «химчистка ковров» → /uslugi/kovry',
    min: 15, max: 400
  },
  priority_groups: {
    label: 'Приоритетные группы',
    formula: 'Какие группы запускать первыми — от горячего к холодному, не наоборот',
    example: 'Сначала горячие группы (заказать, вызвать) — конверсии при минимальном бюджете',
    min: 15, max: 400
  },
  demand_volume: {
    label: 'Оценка объёма спроса',
    formula: 'Через Прогноз бюджета Яндекс Директа. Минимум 50 кликов/неделю на группу',
    example: '12 000 запросов/мес в Москве, ~600 кликов/мес при CTR 5%',
    reference: 'Если меньше — расширить семантику или объединить группы',
    min: 15, max: 400
  },
  forecast_cpa: {
    label: 'Прогноз CPA / CPL',
    formula: '[Расчётный CPL] + [логика: CPC × CR] + [сравнение с целевым]',
  ...
  search_logic: {
    ...
    formula: '5–10 фраз одного смысла на группу. Меньше — слабое обучение, больше — потеря релевантности',
    example: 'Ключи: химчистка дивана заказать. Минусы: своими руками. Объявление: 2–3 варианта на группу.',
    reference: 'Только длиннохвостые фразы 3–6 слов. Без пересечений между группами',
    min: 30, max: 600
  },
  display_logic: {
    label: 'РСЯ / Display: аудитории и креативы',
    formula: 'Тематические слова + интересы и привычки + сегменты аудитории — не копировать поисковые фразы',
    example: 'Аудитория: посещали конкурентов + интерес «уборка». Креатив: до/после с ценой.',
    reference: 'В сетях ключи определяют тематику площадок, а не буквальный поиск',
    min: 20, max: 600
  },
  assets_logic: {
    label: 'Дополнения / Assets',
    formula: 'Быстрые ссылки (заголовок ≤30 + описание ≤60) + уточнения (≤25 симв, 12 шт) + callouts',
    example: 'БС: Цены, Отзывы, Примеры работ. Уточнения: Выезд за 2 часа, Гарантия.',
    min: 15, max: 500
  },
  launch_ready: {
    label: 'Что готово к запуску',
    formula: 'Перечень: группы + ключи + минусы + объявления + посадочные — всё для запуска без повторного разбора',
  ...
  handoff: {
    ...
    formula: 'Финальная таблица, достаточная для запуска кампании без повторного разбора семантики',
  ...
  aov: {
    ...
    formula: 'Сколько приносит одна продажа до рекламных расходов',
    example: '4 500₽ за одну услугу химчистки дивана',
    reference: 'Понятен средний чек, источник данных и период оценки',
    min: 3, max: 200
  },
  source: {
    label: 'Источник данных',
    formula: '[Откуда цифры: CRM / бухгалтерия / аналитика] + [период] + [размер выборки]',
  ...
  marginPercent: {
    label: 'Маржа %',
    formula: 'Сколько остаётся после себестоимости и переменных расходов',
  ...
  targetDrr: {
    label: 'Целевой DRR',
    formula: 'Безопасная доля выручки, которую можно отдавать рекламе',
    example: '15% — при марже 45% и потолке затрат 30% от прибыли',
    reference: 'DRR × AOV = допустимый CPA. DRR выше маржи = убыточная реклама',
    min: 3, max: 200
  },
  limitReason: {
  ...
  allowedCpa: {
    ...
    formula: 'Сколько можно платить за клиента: маржа × DRR или прямой расчёт',
    example: '675₽ — маржа 2 025₽ × DRR 33%',
    min: 3, max: 200
  },
  allowedCpl: {
    label: 'Допустимый CPL',
    formula: 'Сколько можно платить за заявку: CPA × конверсия из лида в продажу',
    example: '675₽ × 25% = 169₽ — максимальная стоимость заявки',
    min: 3, max: 200
  },
  promoteAllowed: {
    label: 'Что можно продвигать',
    formula: 'Направления, где экономика положительна — отделить от опасных',
    example: 'Химчистка диванов, ковров — CPL укладывается, маржа > 40%',
    min: 10, max: 400
  },
  promoteForbidden: {
    label: 'Что нельзя продвигать',
    formula: 'Направления, где CPA не окупается — с обоснованием',
  ...
  heroH1: {
    label: 'H1 героя',
    formula: '[Деятельность] + [Предмет/Сегмент] + [Гео] — за 3 сек объяснить кто / что / для кого',
    example: 'Химчистка мебели в Москве — результат за 2 часа',
  ...
  heroVisual: {
    label: 'Визуал героя',
    formula: 'Лучший пример работы — фото до/после или видео. Не стоковое фото',
    example: 'Фото «до/после» — чистый белый диван, мастер в форме',
    min: 5, max: 200
  },
  heroProof: {
    label: 'Соцдоказательство',
    formula: 'Рейтинг ★ + 1–2 статусных факта (бренды, цифра клиентов)',
    example: '★ 4.9 на Яндексе · 1 200+ клиентов · Гарантия результата',
    min: 5, max: 200
  },
  heroMiniBlocks: {
    label: 'Мини-блоки',
    formula: 'Доставка / сроки / гарантия / оплата — 3–4 иконки с коротким фактом',
  ...
  aboutTitle: {
    ...
    formula: '«Почему [Название]» — продать экспертность, а не услугу',
    example: 'Почему iCleaning — 1 200 клиентов за 3 года',
    min: 5, max: 80
  },
  aboutFacts: {
    label: '3 фактоида',
    formula: 'Лет на рынке / объём / ключевой дифференциатор — конкретные цифры, не прилагательные',
    example: '1 200 клиентов · 4.9 средняя оценка · 3 года на рынке',
    min: 10, max: 300
  },
  aboutText: {
    label: 'Текст о подходе',
    formula: '2–3 предложения о сути подхода, не общие слова о «качестве»',
  ...
  cases: {
    label: 'Кейсы',
    formula: '3–4 визуально сильных примера из разных категорий: Название + Категория + Результат',
  ...
  processTitle: {
    ...
    formula: '«Как мы работаем» — снять страх «это долго и сложно»',
    example: 'Как мы работаем — 4 простых шага',
    min: 5, max: 80
  },
  processSteps: {
    label: 'Шаги процесса',
    formula: '3–5 шагов: название шага + что происходит + срок',
  ...
  benefits: {
    label: 'Выгоды',
    formula: '3 иконки + тезисы: конкретная выгода клиента, не «мы лучшие»',
    example: 'Результат за 2 часа · Гипоаллергенная химия · Гарантия 30 дней',
    reference: 'Задача: сформировать потребность у холодного посетителя',
    min: 15, max: 400
  },
  reviews: {
    label: 'Отзывы',
    formula: 'Виджет Яндекс/Google Reviews или 3 цитаты с фото и именем + кнопка «Все отзывы»',
    example: 'Яндекс: 4.9/5, 230 отзывов. «Диван как новый, мастер приехал через час»',
    reference: 'Подтвердить репутацию чужими словами',
    min: 10, max: 500
  },
  reviewsButton: {
  ...
  serviceProblem: {
    ...
    formula: '2–3 формулы: Боль (заголовок) → Решение (2–3 предл.) + мини-доказательства',
    example: 'Пятно на диване не выводится домашними средствами — впитывается глубже',
    reference: 'Зацепить болью и сразу показать выход',
    min: 15, max: 400
  },
  serviceProof: {
    label: 'Доказательства / кейсы',
    formula: '2–4 кейса: До / После / Результат + отзывы с фото клиента',
    example: 'Диван после залития кофе → восстановление за 2 часа, фото до/после',
    reference: 'Закрыть «а вдруг не получится» — реальный результат',
    min: 10, max: 500
  },
  serviceSteps: {
    label: 'Процесс / этапы',
    formula: '3–5 шагов: шаг → ожидаемый результат → срок. Убрать неопределённость',
    example: '1. Осмотр (5 мин) → 2. Обработка (40 мин) → 3. Сушка (1 час) → 4. Проверка',
    reference: 'Клиент должен знать, что будет дальше',
    min: 10, max: 400
  }
};
...
function standardByLabel(label) {
  ...
  if (/продукт.*одно\s*предложение/.test(l)) return 'продукт_одно_предложение';
  if (/сегмент.*ситуаци/.test(l)) return 'сегмент_кто_и_в_какой_ситуации';
  if (/задача\s*клиента/.test(l)) return 'задача_клиента_что_хочет_изменить';
  if (/результат.*состояние|состояние\s*после/.test(l)) return 'результат_состояние_после';
  return null;
}
...
function systemContextBannerHtml(keys, title, forceOpen = false) {
  ...
  if (filled === 0) return '';
  return `<details class="system-context-banner compact-context"><summary>Контекст: ${filled} из ${items.length}</summary><div class="ctx-grid">${items.join('')}</div></details>`;
}
